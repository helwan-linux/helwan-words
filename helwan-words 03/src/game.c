...
gboolean game_next_round(Game *game)
{
    ...
    guint index = g_random_int_range(0, game->all_words->len);
    gchar *selected = g_strdup(g_ptr_array_index(game->all_words, index));
...
gboolean game_guess(Game *game, const gchar *letter)
{
    ...
    if (all_letters_found(game)) {
        game->state = GAME_WON;
        game->score += 2; // منح نقطتين عند الفوز بكل كلمة
...
guint game_get_remaining_guesses(const Game *game)
{
    ...
    return game->max_wrong_guesses - game->wrong_guesses;
}
...
gboolean game_buy_extra_attempt(Game *game)
{
    ...
    game->score -= 10;            // خصم 10 نقاط
    game->max_wrong_guesses += 1; // زيادة محاولة إضافية متاحة
