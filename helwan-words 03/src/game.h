...
typedef struct {
    ...
    guint max_wrong_guesses; // عدد المحاولات الخطأ القصوى الحالية (قابلة للزيادة بالشراء)

    guint score;             // عداد النقاط (كل فوز يمنح نقطتين)

    GameLanguage language;
    GameState state;

    GPtrArray *all_words;    // تخزين كلمات الملف للتبديل التلقائي بدون إعادة فتحه
} Game;
...
gboolean game_next_round(Game *game); // دالة الانتقال التلقائي لكلمة جديدة
...
gboolean game_buy_extra_attempt(Game *game); // دالة شراء محاولة بـ 10 نقاط
